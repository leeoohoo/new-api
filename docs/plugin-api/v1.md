...
export const meta = {
  ...
  protocols: [{name: "openai_responses", supports: ["stream", "sync", "background", "compaction"]}, "openai_video"],
};
```
...
Each `protocols` entry claims a host protocol. A protocol that defines supports must be claimed in object form with an explicit `supports`; the bare-string form is valid only for protocols without supports (`openai_video`). `supports` declares which client request forms and hookless capabilities the plugin accepts on `openai_responses`: `"stream"` (`stream: true`), `"sync"` (neither flag; the call blocks until the terminal Response), `"background"` (`background: true`; the create returns a pending Response immediately), and `"compaction"` (the standard OpenAI Responses compaction endpoint, `POST /v1/responses/compact`). An unsupported request form is rejected with a 400 at channel selection, before any plugin hook runs and before billing. Retrieval (`GET /v1/responses/:response_id`) is not a support: every created response is always retrievable.
...
The host protocol advertises these standard interfaces. The task-plugin bridge
owns create/retrieve/video bindings; Responses compaction is served by the
gateway's standard OpenAI relay route and advertised by the `compaction` support
marker.

| Protocol           | Operation | Binding                   | Body                              |
|--------------------|-----------|---------------------------|-----------------------------------|
| `openai_responses` | create    | `POST /v1/responses`      | JSON                              |
| `openai_responses` | compact   | `POST /v1/responses/compact` | JSON                           |
...
`protocols.openai_responses` always requires `decodeRequest`. The remaining hooks are derived from hook-bearing `supports` values and verified exactly at load in both directions: `"stream"` requires `renderEvents`; `"sync"` or `"background"` requires `renderFinal`; `"compaction"` is a hookless capability marker for the gateway's standard OpenAI Responses compaction route. A hook required by a supported mode but not exported, or an exported hook that no supported mode uses, rejects the plugin. For a plugin supporting only `"stream"`, the host renders retrieval at terminal SUCCESS by calling `renderEvents` once with no previous state — the same first-call-at-terminal semantics streaming already requires. The supports vocabulary is append-only under apiVersion 1: new modes or hookless capabilities may be added (existing manifests are unaffected; a manifest declaring a support loads only on gateways that define it), but a support may never be removed, renamed, or re-mapped to different hooks without an apiVersion bump. `protocols.openai_video` requires `decodeRequest` for create and `render` for retrieve. The host extracts and pins `ctx.model`, normalizes `ctx.stream`, frames SSE, creates failure envelopes, and calls `renderFinal` only for `SUCCESS`.
